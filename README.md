...
показывается комиссия по городу, которая берётся из того же `CityModifier`, что и у товаров,
но без дефолта "нет строки → коэффициент 1" — для услуги отсутствие строки означает "не
оказывается в этом городе". Сайт не хранит валюту игроков — она живёт в Foundry VTT
(нативные поля cp/sp/ep/gp/pp); сюда же относится и справочник приёма номиналов монет по
городу (`CityCoinAcceptance`, `/admin/economy/coin-acceptance`) — обе фичи не считают и не
двигают деньги, только показывают условия. Подробности — в `CLAUDE.md`.
