...
- **Режим «Полностью заменить» при Excel-импорте** (2026-09-10): до этого повторная загрузка
  любого листа (кроме "Оплата заданий") была чистым upsert по ключу — то, чего нет в файле,
  оставалось в БД нетронутым навсегда. Добавлена опция синхронизации: `replaceExisting: true`
  у всех методов `IExcelEconomyImportService` (кроме `ImportQuestPayRatesAsync` — тот и так
  ...
  поведение не меняется) и параметр `AlwaysReplaces` (для "Оплата заданий" — прячет чекбокс,
