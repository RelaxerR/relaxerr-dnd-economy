...
/// в файле нет, удаляются (см. <see cref="EconomyImportSummary.TotalRemoved"/>); листу
/// "Экономическая активность" эта опция не нужна, он не имеет естественного ключа и всегда заменяется полностью.
///
...
public interface IExcelEconomyImportService
{
  ...
  /// <summary>
  /// Импортирует лист "Экономическая активность" — ставки дохода/расхода (зм/день на игрока)
  /// партийных заданий и занятий в простое. В отличие от остальных листов здесь нет
  /// естественного ключа записи (несколько активностей одного Типа+Категории+Уровней — обычное
  /// дело), поэтому загрузка всегда полностью заменяет текущее содержимое таблицы содержимым
  /// листа — параметр <paramref name="replaceExisting"/> присутствует только для единообразия
